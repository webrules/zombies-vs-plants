...
final class GameModelTests: XCTestCase {
    ...
    func testPlantingSpendsSunshineAndOccupiesCell() {
        ...
        XCTAssertEqual(game.sunshine, 1_400)
    ...
    func testCannotPlantTwiceInOneCell() {
        ...
        game.plant(at: cell)
        game.plant(at: cell)
        XCTAssertEqual(game.sunshine, 1_450)
    ...
    func testIcePeaPlacementCostsSunshineAndFiresAVisibleProjectile() {
        ...
        let cell = GridCell(row: 2, column: 1)
        game.plant(at: cell)
        XCTAssertEqual(game.sunshine, 1_350)
        XCTAssertEqual(game.plants[cell]?.kind, .icePeaShooter)
        game.plant(at: GridCell(row: 2, column: 0))
        XCTAssertEqual(game.sunshine, 1_200, "Ice Pea Shooters should have no planting cooldown")
    ...
    func testFlameStakePlacementCooldownAndProjectileConversion() {
        ...
        XCTAssertEqual(game.sunshine, 1_200)
    ...
    func testGatlingPeaShooterRapidBurstHasDamageAndNoPlantingCooldown() {
        ...
        game.plant(at: first)
        game.plant(at: GridCell(row: 2, column: 0))
        XCTAssertEqual(game.sunshine, 800, "Gatling Pea Shooters should be placeable back-to-back")
    ...
    func testNewGameDefaultsToEasyFifteenHundredSunshine() {
        let game = GameModel()
        game.soundEnabled = false
        game.start()
        game.togglePause()
        XCTAssertEqual(game.sunshine, 1_500)
    }
    ...
    func testDifficultyChangesEnemyPressureAndSpeed() {
        XCTAssertEqual(Difficulty.easy.startingSunshine, 1_500)
        XCTAssertEqual(Difficulty.normal.startingSunshine, 1_200)
        XCTAssertEqual(Difficulty.hard.startingSunshine, 1_000)
        XCTAssertEqual(Difficulty.hell.startingSunshine, 800)
        ...
        XCTAssertEqual(hell.schedule.count, easy.schedule.count * 2, "Hell should be exactly two bounded invasions")
    }

    func testDifficultyAddsEconomyHealthAndCooldownPressure() {
        XCTAssertLessThan(Difficulty.hell.sunshineYieldMultiplier, Difficulty.hard.sunshineYieldMultiplier)
        XCTAssertGreaterThan(Difficulty.hell.enemyHealthMultiplier, Difficulty.hard.enemyHealthMultiplier)
        XCTAssertGreaterThan(Difficulty.hell.cooldownMultiplier, Difficulty.hard.cooldownMultiplier)
        XCTAssertLessThan(Difficulty.hell.crowdControlMultiplier, Difficulty.hard.crowdControlMultiplier)
    }

    func testHellSpawnsBothBoundedInvasionsIncludingDancerCap() {
        let game = GameModel()
        game.soundEnabled = false
        game.selectedDifficulty = .hell
        game.spawnScheduledForTesting(until: 70)
        XCTAssertEqual(game.zombies.count, game.schedule.count)
        XCTAssertEqual(game.zombies.filter { $0.kind == .dancerSamurai }.count, 30)
        XCTAssertTrue(game.zombies.filter { $0.kind.isBoss }.count >= 8)
    }

    func testTerrainHazardsAreTelegraphedAndBoundedToPlayableLanes() {
        let game = GameModel()
        game.selectedDifficulty = .hard
        game.activateTerrainHazardForTesting(wave: 2)
        XCTAssertEqual(game.terrainHazard?.kind, .frost)
        XCTAssertFalse(game.terrainHazard?.lanes.isEmpty ?? true)
        XCTAssertTrue(game.terrainHazard?.lanes.allSatisfy { (0..<GameModel.rows).contains($0) } ?? false)
        XCTAssertEqual(game.terrainHazard?.remaining ?? 0, Difficulty.hard.hazardDuration, accuracy: 0.01)
    }
    ...
    func testRedHotPepperClearsOrdinaryLaneAndDamagesBosses() {
        ...
        game.plant(at: cell)
        XCTAssertEqual(game.sunshine, 1_375)
    ...
    func testCharmMushroomTargetsChargesConvertsAndCooldowns() {
        ...
        XCTAssertEqual(game.sunshine, 1_325)
