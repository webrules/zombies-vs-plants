...
enum TerrainHazardKind: String, Sendable, Equatable {
    case wet
    case frost
    case shadow
    ...
    var title: String {
        switch self {
        case .wet: "WET MOSS"
        case .frost: "FROST VEIL"
        case .shadow: "SHADOW BLOOM"
        }
    }

    var plantActionMultiplier: Double {
        switch self {
        case .wet: 0.84
        case .frost: 0.68
        case .shadow: 0.88
    ...
    var zombieSpeedMultiplier: Double {
        switch self {
        case .wet: 0.92
        case .frost: 1.08
        case .shadow: 1.04
...
struct TerrainHazard: Sendable, Equatable {
    let kind: TerrainHazardKind
    let lanes: Set<Int>
    var remaining: Double
}
...
enum Difficulty: String, CaseIterable, Identifiable, Sendable {
    ...
    var enemyHealthMultiplier: Double {
        switch self { case .hell: 1.28; case .hard: 1.16; case .normal: 1.08; case .easy: 1.0 }
    }
    var sunshineYieldMultiplier: Double {
        switch self { case .hell: 0.68; case .hard: 0.80; case .normal: 0.90; case .easy: 1.0 }
    }
    var cooldownMultiplier: Double {
        switch self { case .hell: 1.30; case .hard: 1.18; case .normal: 1.08; case .easy: 1.0 }
    }
    var crowdControlMultiplier: Double {
        switch self { case .hell: 0.62; case .hard: 0.75; case .normal: 0.88; case .easy: 1.0 }
    }
    var bossCooldownMultiplier: Double {
        switch self { case .hell: 0.72; case .hard: 0.84; case .normal: 0.93; case .easy: 1.0 }
    }
    var hazardDuration: Double {
        switch self { case .hell: 11.0; case .hard: 9.0; case .normal: 7.0; case .easy: 4.5 }
    }
    var coordinatesBosses: Bool { self == .hard || self == .hell }
    var dancerWaveCap: Int { self == .hell ? 10 : Self.baseDancerWaveCap }
    private static let baseDancerWaveCap = 5
    var subtitle: String {
        switch self {
        case .hell: "800 sun · Double invasion · Bosses coordinate"
        case .hard: "1,000 sun · Armored waves · Faster cooldowns"
        case .normal: "1,200 sun · Denser waves · Hazard lanes"
        case .easy: "1,500 sun · Learn the garden · Clear telegraphs"
        }
    }
}
...
final class GameModel: ObservableObject {
    ...
    static let startingSunshine = Difficulty.easy.startingSunshine
    ...
    @Published private(set) var terrainHazard: TerrainHazard?
    @Published private(set) var adaptivePressure = 0
    ...
    var schedule: [Spawn] {
        ...
        case .easy:
            return baseSchedule
        case .normal:
            return pressuredSchedule(extraStride: 5)
        case .hard:
            return pressuredSchedule(extraStride: 3)
        case .hell:
            // Hell is a bounded, literal two-wave invasion rather than an infinite mode.
            let secondWave = baseSchedule.map { spawn in
                Spawn(time: spawn.time + 0.28, row: (spawn.row + 1) % Self.rows, kind: spawn.kind)
            }
            return (baseSchedule + secondWave).sorted { $0.time < $1.time }
        }
    }

    private func pressuredSchedule(extraStride: Int) -> [Spawn] {
        let extras = baseSchedule.enumerated().compactMap { index, spawn -> Spawn? in
            guard index % extraStride == 0, !spawn.kind.isBoss, spawn.kind != .dancerSamurai else { return nil }
    ...
    func plant(at cell: GridCell) {
        ...
        if kind == .cherryBomb {
            ...
            sunshine -= kind.cost
            cherryCooldown = Self.cherryCooldownDuration * selectedDifficulty.cooldownMultiplier
        ...
        if kind == .redHotPepper {
            ...
            sunshine -= kind.cost
            pepperCooldown = Self.pepperCooldownDuration * selectedDifficulty.cooldownMultiplier
        ...
        if kind == .charmMushroom {
            ...
            sunshine -= kind.cost
            charmCooldown = Self.charmMushroomCooldownDuration * selectedDifficulty.cooldownMultiplier
    ...
    func activateTerrainHazardForTesting(wave: Int) {
        activateTerrainHazard(for: wave)
    }
    ...
    private func update(dt: Double) {
        ...
        if var hazard = terrainHazard {
            hazard.remaining -= dt
            terrainHazard = hazard.remaining > 0 ? hazard : nil
        }
        ...
        for cell in Array(plants.keys) {
            ...
            let hazardRate = terrainHazard?.lanes.contains(cell.row) == true
                ? terrainHazard?.kind.plantActionMultiplier ?? 1.0
                : 1.0
            let actionRate = (plant.freezeTimer > 0 ? 0.32 : 1.0) * hazardRate
            ...
            if plant.kind == .sunflower, plant.actionTimer <= 0 {
                let hazardYield = terrainHazard?.lanes.contains(cell.row) == true
                    ? terrainHazard?.kind.sunshineMultiplier ?? 1.0
                    : 1.0
                sunshine += max(1, Int((25.0 * selectedDifficulty.sunshineYieldMultiplier * hazardYield).rounded()))
                plant.actionTimer = 7
        ...
        for zi in zombies.indices {
            ...
            if let pea = peas.filter({ $0.row == zombies[zi].row && !hitPeas.contains($0.id) })
               ...
               abs(pea.x - zombies[zi].x) < 0.14 {
                ...
                if pea.icy {
                    zombies[zi].slowTimer = Self.icePeaSlowDuration * selectedDifficulty.crowdControlMultiplier
        ...
        for zi in zombies.indices.reversed() {
            ...
            if zombies[zi].kind == .thunderShogun {
                if zombies[zi].strikeCharge > 0 {
                    ...
                    if zombies[zi].strikeCharge <= 0 {
                        ...
                        zombies[zi].attackCooldown = 4.5 * selectedDifficulty.bossCooldownMultiplier
                    }
                    continue
            ...
            if zombies[zi].kind == .iceDoctor, !isBlocking {
                if zombies[zi].strikeCharge > 0 {
                    ...
                    if zombies[zi].strikeCharge <= 0 {
                        ...
                        zombies[zi].attackCooldown = 4.8 * selectedDifficulty.bossCooldownMultiplier
            ...
            if let cell = target, isBlocking {
                    ...
                    if zombies[zi].kind == .hammerGiant {
                        if zombies[zi].strikeCharge <= 0, zombies[zi].attackCooldown <= 0 {
                            ...
                            if zombies[zi].strikeCharge <= 0 {
                                plant.hp -= 285
                                plant.hitFlash = 0.35
                                zombies[zi].attackCooldown = 1.65 * selectedDifficulty.bossCooldownMultiplier
                                screenShake = 0.42
                        ...
                        if zombies[zi].strikeCharge <= 0, zombies[zi].attackCooldown <= 0 {
                            ...
                            if zombies[zi].strikeCharge <= 0 {
                                ...
                                plant.hitFlash = 0.35
                                zombies[zi].attackCooldown = 2.1 * selectedDifficulty.bossCooldownMultiplier
                                screenShake = 0.2
                ...
                let hazardSpeed = terrainHazard?.lanes.contains(row) == true
                    ? terrainHazard?.kind.zombieSpeedMultiplier ?? 1.0
                    : 1.0
                let adaptiveSpeed = 1.0 + Double(adaptivePressure) * 0.035
                zombies[zi].x -= zombies[zi].kind.speed * selectedDifficulty.enemySpeedMultiplier
                    * adaptiveSpeed * danceSpeed * slowMultiplier * hazardSpeed * dt
            }
        }
    ...
    private func spawnDueZombies() {
        while nextSpawn < schedule.count, schedule[nextSpawn].time <= elapsed {
            ...
            if spawn.kind == .dancerSamurai {
                ...
                guard dancerSpawnCountsByWave[spawnWave] < selectedDifficulty.dancerWaveCap else { continue }
                dancerSpawnCountsByWave[spawnWave] += 1
            }
            var zombie = Zombie(kind: spawn.kind, row: spawn.row,
                                hp: spawn.kind.maxHP * selectedDifficulty.enemyHealthMultiplier)
            if spawn.kind == .iceDoctor {
                zombie.attackCooldown = 2.6 * selectedDifficulty.bossCooldownMultiplier
                audioIfEnabled(.doctorArrival)
            }
            if spawn.kind == .flameGiant {
                zombie.attackCooldown = 2.2 * selectedDifficulty.bossCooldownMultiplier
                audioIfEnabled(.flameArrival)
            }
            if spawn.kind == .thunderShogun {
                zombie.attackCooldown = 3.0 * selectedDifficulty.bossCooldownMultiplier
                audioIfEnabled(.thunderArrival)
            }
            zombies.append(zombie)
        }
    }

    private func updateAdaptivePressure() {
        guard selectedDifficulty != .easy else {
            adaptivePressure = 0
            return
        }
        let expectedPlants = max(1, wave * 2)
        let defenseIsStable = plants.count >= expectedPlants && sunshine > selectedDifficulty.startingSunshine / 3
        if defenseIsStable {
            adaptivePressure = min(2, adaptivePressure + 1)
        } else {
            adaptivePressure = max(0, adaptivePressure - 1)
        }
    }

    private func activateTerrainHazard(for wave: Int) {
        let lanes: Set<Int>
        if selectedDifficulty == .easy {
            lanes = [max(0, min(Self.rows - 1, wave - 1))]
        } else if waterLanes.isEmpty {
            lanes = [wave % Self.rows]
        } else {
            lanes = selectedDifficulty == .hell ? waterLanes : [waterLanes.first!]
        }
        let kind: TerrainHazardKind = switch wave {
        case 1: .wet
        case 2: .frost
        default: .shadow
        }
        terrainHazard = TerrainHazard(kind: kind, lanes: lanes, remaining: selectedDifficulty.hazardDuration)
    }

    private func coordinateBossesIfNeeded() {
        guard selectedDifficulty.coordinatesBosses else { return }
        for index in zombies.indices where zombies[index].kind == .iceDoctor && !zombies[index].charmed {
            if zombies[index].strikeCharge <= 0, zombies[index].attackCooldown <= 0 {
                zombies[index].strikeCharge = 1.4
                audioIfEnabled(.iceCharge)
            }
        }
    }
    ...
    private func triggerCharm(row: Int, targetID: UUID?) -> Bool {
        ...
        zombies[index].charmTimer = 8.0 * selectedDifficulty.crowdControlMultiplier
    ...
    private func fireCorn(at target: GridCell) {
        ...
        cornCooldown = Self.cornCannonCooldownDuration * selectedDifficulty.cooldownMultiplier
        cornTarget = target
